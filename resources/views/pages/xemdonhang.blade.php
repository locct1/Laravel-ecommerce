    ...
    <tbody>
      ...
      <tr>
        ...
        <td>
          ...
          <form action="">
            @csrf
            <span class="badge badge-danger" id="choxuly_{{$dh->dh_id}}">Đang chờ xử lý</span>
          </form>
          @elseif($dh->dh_status==1)
          <span class="badge badge-primary" id="danggiaohang_{{$dh->dh_id}}">Đang giao hàng</span>
          @else
        ...
        <td>
          <form action="">
            @csrf
            <button type="button" class="btn btn-primary xemchitiet" data-button="{{$dh->dh_id}}" data-toggle="modal" data-target="#exampleModal" data-whatever="@getbootstrap"><i class="fa fa-eye"></i> Xem chi tiết</button>
          </form>
