                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <form action="">
                                @csrf
                                <a type="button" id="dh_{{$dh->dh_id}}" class="btn-capnhat-danggiaohang" data-danggiaohang="{{$dh->dh_id}}"> <span class="badge badge-danger" id="choxuly_{{$dh->dh_id}}">Đang chờ xử lý</span></a>
                            </form>
                            @elseif($dh->dh_status==1)
                            <a type="button" class="btn-capnhat-dagiaohang" data-dagiaohang="{{$dh->dh_id}}"> <span class="badge badge-primary" id="danggiaohang_{{$dh->dh_id}}">Đang giao hàng</span></a>
                            @else
                        ...
                        <td>
                            <div id="ngaycapnhat_{{$dh->dh_id}}">{{date('d/m/Y H:i:s', strtotime($dh->updated_at))}}</div>
                        </td>
                        <td>
                            <a href="admin/donhang/indonhang/{{$dh->dh_id}}" class="btn btn-success"><i class="fa fa-print"></i></a>
                            <a href="admin/donhang/chitietdonhang/{{$dh->dh_id}}" class="btn btn-primary"><i class="fa fa-eye"></i></a>
                            <a data-dh_id='{{$dh->dh_id}}' class="btn btn-danger btnDelete"><i class="fa fa-trash-alt"></i></a>
                        </td>
                    </tr>
                    @endforeach
                </tbody>
        ...
        function capnhatDangGiaoHang(id) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $('#choxuly_' + data.dh_id).removeClass('badge-danger').addClass('badge-primary');
                    $('#choxuly_' + data.dh_id).html('Đang giao hàng');
                    $('#dh_' + data.dh_id).attr('data-dagiaohang', data.dh_id);
                    $('#dh_' + data.dh_id).removeClass('btn-capnhat-danggiaohang').addClass('btn-capnhat-dagiaohang');
                    $('#choxuly_' + data.dh_id).attr("id", 'danggiaohang_' + data.dh_id);
                    
                    $('#ngaycapnhat_' + data.dh_id).html($.format.date(data.updated_at, 'dd/MM/yyyy HH:mm:ss'));
        ...
        function capnhatDaGiaoHang(id) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $('#danggiaohang_' + data.dh_id).removeClass('badge-primary').addClass('badge-success');
                    $('#danggiaohang_' + data.dh_id).html('Đã giao hàng');
                    $('#ngaycapnhat_' + data.dh_id).html($.format.date(data.updated_at, 'dd/MM/yyyy HH:mm:ss'));
                },
                error: function(jqXHR, textStatus, errorThrown) {
                    console.log(textStatus, errorThrown);
